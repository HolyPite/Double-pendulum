...
- `js/sims/*.js` — one file per simulation: `pendulum.js`, `fluid.js`, `sand.js`, `boids.js`, `particlelife.js`, `grayscott.js`, `gameoflife.js`, `waves.js`, `physarum.js`, `ants.js`. Each is an IIFE that calls `Engine.register({...})`; no globals leak except via the registry.
...
**Other sims:** `fluid.js` is a Jos Stam "Stable Fluids" solver (128² grid, Float32Arrays, vorticity confinement, RGB dye advected and rendered via an offscreen ImageData stretched to screen). `sand.js` is a falling-sand cellular automaton (4px cells, Uint8Array, bottom-up scan for falling materials and top-down for rising ones so a moved cell is never processed twice per frame; alternating scan direction for symmetry). `boids.js` is classic Reynolds flocking, O(n²) neighbor search (fine ≤ 500 boids — add spatial hashing before raising the cap). `particlelife.js` uses a spatial grid (Int32Array linked lists) and a toroidal world; its interaction matrix editor lives in the panel. `grayscott.js`, `gameoflife.js`, `waves.js`, `physarum.js`, `ants.js` all follow the same grid pattern: typed arrays at a reduced resolution (`SCALE` px/cell), offscreen canvas + ImageData, stretched to screen (smoothing on for continuous fields, off for cellular ones). Grids wrap toroidally except waves and ants (bounded). Right-click is an eraser tool where relevant (the engine suppresses the context menu). `ants.js` is a stigmergic ant-colony model: per-species dual pheromone fields (home/food) obeying `∂τ/∂t = Δτ + D∇²τ − ρτ` (deposit + 4-neighbour Laplacian diffusion that treats wall neighbours as zero-flux + evaporation), agents with decaying deposit strength so trails form gradients toward the source; nests are stored in pixel coords (survive nothing — grids reset on resize), food/obstacles live in the grid, up to 3 competing species with a live food-collection scoreboard.

A common pitfall on Windows PowerShell 5.1: don't rewrite UTF-8 source files via `Get-Content | Set-Content` — it mangles accented characters (no BOM detection). Use the Edit/Write tools instead.
